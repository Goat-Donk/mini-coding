...
4. **样本量**：**已计分的** 21 个有效任务全部来自 tinydb **一个仓库、一个语言、一个测试风格**。完成率**不能外推**到别的语言/仓库；它证明的是「这套流程在本仓库上跑得通」。本轮把**多仓参数化**做完了、第二个仓库（sqlparse）的**闸门也跑完了**（143 个候选 → 54 个有效），所以「两个仓库共 75 个有效任务」是**闸门侧的实测**；但**计分侧的样本仍然只有一个仓库** —— 第二仓的付费三臂重跑还没跑，这两句话不能合并成一句讲。
...
| 评估 | `eval/golden_tasks.py` `runner.py` | 1,870 |
| **源码合计** | **31 个模块** | **14,425** |
...
| 评测 | 三臂真跑（`agent` 66.7% / `one-step` 0% / `single-shot` 84.2% 反事实 → **89.5% 实测**），报告带 `ruler` 指纹自证「哪把尺子」 |
| 评测花费（**含一次跑错的**） | 本轮两次付费运行合计 **¥0.5827**（预授权 ¥0.85，未超）：第一次 `--limit 21` **是我把候选数当成了任务数** —— 那不是 21 个任务，是 21 个候选（→ 13 个有效），它没有把 84.2% 变成实测值，**¥0.2374 白花**；纠正 `--limit` 帮助文案后重跑 `--limit 39` 才是留档那份，**¥0.3453**。两次的 `runner_sha256` 不同，报告里可分辨 |
...
2. **样本量小。** 三臂评测**计分侧只有一个仓库、一个语言、一个测试风格**（tinydb）。本轮把多仓参数化做完、第二个仓库（`sqlparse`）的闸门也跑完了（143 候选 → 54 有效），但**第二仓的付费三臂没跑** —— 所以**样本仍然不能外推**。
3. **一份**事实**，不作判断**：本仓 git 历史跨 **2026-09-10 ~ 2026-09-12、68 个提交**，而 `TASKS.md` 的里程碑标题写的是 **Day 1–21**（首提交是「初始交接文档」）。**我只报这两个数**，不解释成因，也不据此声称任何关于工作量的结论。
