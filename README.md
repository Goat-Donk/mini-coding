...
**成本对不上，如实记下、不解释**：同一批任务、几乎相同的 token（341,337 vs 337,428），归档那轮 **0.0% 缓存命中 / ¥0.8423**，这次 **98.6% / ¥0.3453**，差 2.4 倍。`Usage.cache_hit_ratio` 在 hit+miss == 0 时返回 `None`，平均 0.0% 意味着 DeepSeek **当时确实返回了 hit=0**（不是没记）。**原因不明，只报事实**；且所有 ¥ 都按 `archived` 的定价快照估算，**是量级估算，不是账单**。
