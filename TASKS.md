...
## M5-7 第二仓真跑：跨仓泛化对比（2026-09-12/13）★「换一个仓库，结论会不会变」

**起点**：M5-6 已把第二仓 `sqlparse` 的**闸门**跑通（零成本），但三臂真跑**停下来等点头**了。本轮把三臂跑完，产出跨仓对比。**交付物是 `docs/eval_cross_repo_comparison.md`**（追踪在版本库里），本节记过程与教训。

- [x] **第二仓三臂真跑**（`--repo sqlparse`，与第一仓同模型、同定价快照、同 `pytest`/`python`）
  - `one-step` `report-20260912-235024.json`：19 有效 / judged 19 / 通过 **0** / 38,327 token / ¥0.0860 / 命中率 **0.000**
  - `single-shot` `report-20260913-004054.json`：19 有效 / judged **11** / 通过 **5** / 808,358 token / ¥2.0893 / 命中率 **0.000**
  - `agent` `report-20260913-014117.json`：19 有效 / judged 19 / 通过 **5** / 2,808,714 token / ¥2.0160 / 命中率 **0.926**
  - 三臂 `judge_guard=True`、`ruler` 统一 `a3e7c441aee2` / `4b226f39befd` —— **第二仓是同一把尺子**。
- [x] **三臂可比性校验通过**：`--repo sqlparse` EXIT=0，7 项 ✓ + 1 条 ⓘ（第二仓**没有留档的闸门日志**作外部锚 —— 那是「未检查」，不是「不通过」，可信度改由跨臂互证承担：三条臂各自独立跑闸门，得到逐个一致、顺序一致的同一批任务集）。
- [x] **`drive_three_arms.py` 的写死分母已修**（6 处）：原先写死 `21`（第一仓的有效任务数）且**真参与计算**，换仓后同一份输出自相矛盾（2a 打 5/21 = 23.8%、2b 打 5/19 = 26.3%）。改为从数据派生。**双向回归**：第二仓修正真错误、**第一仓数字零变化**（14/21、20/21、19/21 一个没动）。
- [x] **两条工程教训（都不是产品代码的 bug，是评测基础设施的）** —— 详见 `docs/TECH_SPEC.md` §9.14
  1. **`subprocess.run(timeout=…)` 在 Windows 上有清道夫死锁**：超时后只 `kill()` 直接子进程（`cmd /c` 壳），随后**再调一次 `communicate()` 排空管道**，而孙进程仍握着写端 ⇒ 永久阻塞。**bash 工具的 120 s 超时形同虚设**，本轮 `agent` 臂为此白烧 9 次超时（556/319/255/231/187/136/135/134/134 s，合计 ≈2087 s，全部 `exit_code=None`）。直接证据：`Get-CimInstance` 遍历 runner 子进程树**为空**，孤儿的 `ParentPID` 指向**已死的壳**。
  2. **`_run_pytest` 完全没有超时**（`eval/golden_tasks.py:545-548`）：判定器一旦遇到非终止测试就**永久钉死整条臂**。本轮实测任务 `8b034273` 的补丁在 `group_comments` 里把 `eidx` 退到 `tidx` 之前 → `token_next_by(T.Comment)` 每次返回同一个 token ⇒ 非终止。`probe772.py` 15 s 硬超时：**补丁版未结束、`git HEAD` 的 base 版毫秒级 `rc=0`** ⇒ **死循环是补丁自己造的，不是 base 的 bug**（我先前判错过，已纠正并留痕）。
- [x] **一次运维干预，如实留痕**：判定器 pytest 挂死 300 s+，**外部 `taskkill`（PID 110652）终止**后 runner 推进到 #19 并落地。⇒ 该任务记 `passed=False` —— **该判定为真**（补丁确实不通过），但**归档报告本身不含「被外部终止」的痕迹**，这是本轮的取证缺口。**不改任何评测代码、不影响输出格式**（这是当时与用户确认过的处置边界）。
- [x] **跨仓成本倍率的实测分解**（本轮的正面产出）：`agent` 臂跨仓是**干净的受控对照**（两仓同为热）—— 单价 **1.000×**、token 0.971×、成本 0.971×；`single-shot` **冷–冷对齐**的 **2.48×** 几乎全部来自体积（token 2.37× × 单价 1.047×）；缓存冷热在**同仓内**能单独造成 **2.41×** 的单价差。⇒ **本轮没有「跨仓更贵」的证据**。
- [x] **作废两个无据数字**：早先记的「体积 2.65×」与「6.69× = 2.65 × 2.53」**无法从任何一份归档报告复现**，已删除，替换为实测的三把尺子（源码段 2.52× / 整仓 `.py` 2.10× / 包 `.py` 2.02×）。
- [x] **README 里那条可证伪的预测已兑现**：它为第二仓预测了「`agent` 会热、`one-step` 会热、`single-shot` 会冷」→ 实测 **对 2 错 1**（`one-step` 是 **0.000 冷**）。**错在把「system prompt 相同」当成了「前缀会命中」，漏了时间变量** —— 第二仓 `one-step` 跑在 23:50、第一仓上一轮结束在 21:01，**隔了 2 小时 49 分，provider 侧缓存已过期**。⇒ 顺带拿到**缓存 TTL 过期的第二个独立证据点**；结论写进 README：跨仓比成本除了「发没发过这段字节」，还要问「隔了多久」。
- [ ] **留待下一轮（本轮明确不修，理由是不让已归档的数字失去可复核性）**
  - **`render_task_text` 把仓库名写死成 `tinydb`**（`eval/golden_tasks.py:229`）：第二仓**三条臂**收到的任务文本全都自称「tinydb 仓库」（第一仓不受影响）。**改了它这三份报告的 `ruler` 指纹立刻失效，必须重跑才能引用新数字**，所以列为下一轮第一件事。
  - `_run_pytest` 加超时，且**超时单列一档**（同 `JudgeResult.executed` 的三态教训：算通过和算没通过都是错的），**不要**让它退化成「测试没过」。
  - `subprocess.run` 换显式 `Popen` + 超时后 `kill` 整棵进程树（Windows 用 `taskkill /T`）—— **根因未修，本轮只做了运维层处置**。
  - **已知偏差（用户给定的原话，必须与本表同时引用）**：「8 个任务因 max_tokens=8192 触顶截断，导致 diff 生成中断，产生假阴性。有效评估集实际为 11 个（命中率 5/11 = 45.5%），该数字严重低估了模型真实能力。」**取证边界**：截断**有据**（用已修好的 `_extract_diff` 对那 8 条 `raw_output` 零 LLM 重抠：4 条抠出 0 字节、3 条断在 hunk/token 中间、1 条收在自然边界 ⇒ **7/8 带截断指纹**），但**「8192」这个上限数字无据**（`llm.py:230` 取到了 `finish_reason`，`runner.py:554` 没往下传；且**全仓从不设置 `max_tokens`**，撞的是 API 默认值）。
...
- 代码状态：**14,437 行源码 / 31 模块 / 847 测试全绿**（`python -m pytest tests/ -o addopts="" -q` → `847 passed`，无 skip；M9-8 完成时那一次 **81.36s**、2026-09-12 文档同步后 **88.47s**、M5-5 完成时 **295.86s**、**M5-6 完成时 459.56s**、**M5-7 后全面复验 529.78s** —— **用例数稳定，墙钟随机器负载浮动**；M5-5 那次机器上并发跑过别的东西、M5-6 那次后台跑且机器同时在忙、M5-7 复验那次前 10 分钟有另一个 pytest 并发在跑，都不要拿它们当基线）
  - ⚠️ **2026-09-13 复验时更正**：此前四处文档写的 **14,425 是错的**（`4a1973a` 写下它的时候实际已是 **14,431**，`0f2422e` 又 +6 → **14,437**）。**模块数 31 与测试数 847 经复算无误**（35 个 `.py` 减 4 个空 `__init__.py`；`tests/` 30 个文件 = 23 个 `test_*.py` + 7 个夹具、29 个非空、16,363 行）。已同步 `README.md`（第 7 行 + 规模表）、`CLAUDE.md`、`docs/interview_guide.md`、本文件。
  ...
  - M5-5 的验证脚本在 `evalverify/`（**默认 gitignore**，同 `m9verify/` 惯例；**其中被已提交文档引用为证据或数据源的 `evalverify/` 11 个文件（9 个脚本 + 1 份日志 + 1 份重算报告）与 `data/eval/` 7 份归档报告已 `git add -f` 进版本库** —— 否则文档引用的路径在别的机器上不存在）：`diff_extract_fixed.py`（**已降级为第 0 阶段自校验用的历史 bug 复现**；修复已回写进版本库里的 `eval/runner.py`，见 §9.6d）/ `rescore_single_shot.py`（离线重算，零 LLM）/ `reanalyse_patch_failed.py`（逐任务取证）/ `drive_three_arms.py`（三臂大对照）/ `drive_isolation_report.py` / `drive_judge_gaming.py` / `mutate_m5_5.py` + 各步日志与修正版报告 JSON。
