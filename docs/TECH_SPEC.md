...
`raw_output` 里裁一段内联进测试——夹具**刻意内联、不读归档文件**：一是 `data/` 默认被
gitignore，二是 1.5 MB 的归档不该成为一个单测的依赖），因为
...
- ⚠️ **`evalverify/` 默认被 gitignore**（同 `m9verify/` 惯例），但**被已提交文档当证据或
  数据源引用**的那些脚本与报告已 `git add -f` 进版本库（清单见
  `docs/eval_cross_repo_comparison.md` 文首）—— 否则文档引用的路径在别的机器上不存在。
  它们产出的**结论**（修正值、归因、口径边界）**全部写进了**
