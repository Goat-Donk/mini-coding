...
**M7 安全机制（措辞红线，别写错）**：本项目**没有**做「防止 prompt 注入」。分三层：① `permissions.py` 执行（确定性）② `hooks.py` PreToolUse 阻断（确定性）③ `security.py` 检测（**概率性，只出告警，从不直接决定放行/拒绝**）。检测出 `high` 后收紧能力的是**后置天花板**（`_apply_taint_ceiling`，只把三类不可逆动作 网络外发/读凭据/写记忆文件 从 ALLOW 降为 ASK），位置**必须在 `_always`/`_turn` 之后、`confirm` 之前**。禁用措辞：不说「防御/防止注入」，不说「污点追踪/taint 传播」（实为**会话级粗粒度标记**），不说「纵深防御/零信任」，不说「子代理沙箱」（实为**受限只读工具集**），不给「误报率低」这类无数字形容词。局限逐条写在 README「已知未修复的绕过路径」（S1–S49，其中 S6–S10 配了探针实测、S17–S20 是 M9-6 目标完成检查的代价、S43–S49 是 M5-6 修掉 S16 之后换上来的一批 bash 文本沙箱边界），改任何一条都要同步改那张表。**bash 的凭据判据是「提到即命中」**（`CREDENTIAL_MENTION`，不锚定末尾，见 permissions.py 的注释）——别「顺手修回去」，那会让天花板重新空转。

> ✅ **真实跑分走的是项目选定通路（DeepSeek 官方）**：`https://api.deepseek.com` + `deepseek-chat`，与代码默认值、`.env.example` 一致。
> 最新一次：`python -m eval.runner --limit 39`（三臂：`agent` / `one-step` / `single-shot`）→ 闸门后 **21 个有效任务**，完成率 **`agent` 14/21 = 66.7%** / **`one-step` 0/21 = 0%** / **`single-shot` 16/19 = 84.2%（反事实修正值）**，另有解析器修好后同一批重跑的**实测 17/19 = 89.5%**。真·冷启动曲线 step1 0% → 累计 77%。
>
> **第二仓 `sqlparse`（M5-7，2026-09-12/13）**：`--repo sqlparse`，同模型同定价快照同 `pytest`/`python`，三臂 `judge_guard=True`、`ruler` 统一。**19 个有效任务**：`agent` **5/19 = 26.3%** / `one-step` **0/19 = 0%** / `single-shot` **5/11 = 45.5%**（分母 11 = 19 − 8 个补丁未落地）。**完整跨仓对比见 [`docs/eval_cross_repo_comparison.md`](docs/eval_cross_repo_comparison.md)** —— 引用前先读它的 §1「三处口径差」：**两仓的守卫口径（`null/False` vs `True×3`）、`runner_sha256`、闸门淘汰率（46% vs 5%）、分母全都不同，通过率不许并栏、不许相减。**
> ⚠️ **跨仓成本的正面对着结论**：`agent` 臂跨仓是**干净的受控对照**（两仓同为热缓存）—— 单价 **1.000×**、token 0.971×、成本 0.971×；`single-shot` **冷–冷对齐**的 2.48× 几乎全部来自**体积**（token 2.37× × 单价 1.047×）；缓存冷热在**同仓内**能单独造成 **2.41×** 的单价差。⇒ **没有「跨仓更贵」的证据。**
> ⚠️ **两个曾流传的数字已作废**：「体积 2.65×」「6.69× = 2.65 × 2.53」**从任何归档报告都复现不出来**，替换为实测三把尺子（`single-shot` 实际喂入的源码段 **2.52×** / 整仓 `.py` 含 tests **2.10×** / 包 `.py` 不含 tests **2.02×**），引用时必须标明是哪一把。
> ⚠️ **README 那条预测对 2 错 1**：预测第二仓 `one-step` 会热（理由：system prompt 相同），实测 **0.000 冷** —— 错在**漏了时间变量**（两仓之间隔了 2 小时 49 分，provider 侧缓存已过期）。⇒ 跨仓比成本除了「这段字节发没发过」，**还要问「隔了多久」**。
...
> ⚠️ **`single-shot` 的完成率有三个数，口径各不相同，别混讲**：留档 **52.4%（11/21）** / 离线重算修正 **84.2%（16/19，反事实）** / 解析器修好后重跑**实测 89.5%（17/19）**。**原 52.4% 系评测系统解析器缺陷导致** —— `_FENCE_RE` 开围栏只认 ``` / ```diff / ```patch，模型回复里 diff 块前面的 ```python 当不了开围栏 → **围栏配对整个错位一格** → diff 抠不出来 → 兜底把模型后面的散文整段喂给 `git apply` → `corrupt patch`。**7 个"补丁未应用"里 5 个是这一个 bug 造成的假阴性，不是模型的失败。** **修复已回写进 `eval/runner.py`**（`JUDGE_VERSION` 3 → 4，报告带 `ruler` 指纹），判据从此只有一份且在版本库里；84.2% 是**对归档冻结输出的离线重算**（`evalverify/rescore_single_shot.py`，零 LLM），89.5% 是**另一次采样**，两者之差全部来自采样，详见 README「评估」章节。
